...
@Query("SELECT new org.example.datn_website_supershoes.dto.response.ProductResponse(p.id, p.name, p.productCode, p.imageByte, p.gender, b.id, b.name, c.id, c.name, m.id, m.name, s.id, s.name, p.status) "
        + "FROM Product p JOIN p.brand b JOIN p.category c JOIN p.material m JOIN p.shoeSole s "
        + "WHERE p.status = :status "
        + "AND (:categoryId IS NULL OR c.id = :categoryId) "
        + "AND (:brandId IS NULL OR b.id = :brandId) "
        + "AND LOWER(p.name) LIKE LOWER(CONCAT('%', :name, '%'))")
List<ProductResponse> findProductRequestsByStatus(String status, Long categoryId, Long brandId, String name);
